...
with tf.Graph().as_default() as g:
    (
        ...
        grid_splits,
        out_shape,
    ) = grid_ops.sparse_neighborhood(in_shape, kernel_shape, strides, padding)
    ...
    (event_partitions, event_indices, event_splits) = neigh_ops.compute_full_neighbors(
        in_times, in_coords, out_times, event_duration=100
    )
    ...
    ijs = tf.dynamic_partition(
        ij, tf.cast(event_partitions, tf.int32), num_partitions=9
    )


@tf.function
def f():
    return tf.graph_util.import_graph_def(g.as_graph_def(), return_elements=[ij.name])
