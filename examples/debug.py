...
def run_example(features, labels, grid_shape, num_classes):
    ...
    for _ in range(num_levels):
        (grid_partitions, grid_indices, grid_splits,
         out_shape) = grid_ops.sparse_neighborhood(in_shape, **grid_kwargs)
        ...
        out_times, out_coords = spike_ops.spike_threshold(
            times, coords, grid_indices_T, grid_splits_T, decay_time,
            **spike_kwargs)

        (event_partitions, event_indices,
         event_splits) = neigh_ops.compute_neighbors(
             times,
             coords,
             out_times=out_times,
