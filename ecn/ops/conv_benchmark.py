...
flags.DEFINE_boolean('backward',
                     default=False,
                     help='benchmark forward and backward pass')
flags.DEFINE_integer('burn_iters',
                     default=10,
                     help='number of burn in iterations')
flags.DEFINE_integer('ni', default=100000, help='number of input events')
flags.DEFINE_integer('no',
                     default=-1,
                     help='number of output events. Defaults to ni // 4')
...
def get_kwargs():
    ...
    dt = tuple(
        tf.SparseTensor(ij, val, (n_out, n_in)) for ij, val in zip(ijs, vals))
...
def v1_train_op(features, dt, kernel, decay):
    ...
    dt = [
        tf.SparseTensor(d.indices.numpy(), d.values.numpy(),
                        d.dense_shape.numpy()) for d in dt
    ]
...
def v2_train_op(features, dt, kernel, decay):
    features = tf.constant(features.numpy())
    dt = tf.SparseTensor(dt.indices.numpy(), dt.values.numpy(),
                         dt.dense_shape.numpy())
    with tf.GradientTape() as tape:
        kernel = tf.Variable(kernel.numpy())
        decay = tf.Variable(decay.numpy())
        tape.watch(kernel)
