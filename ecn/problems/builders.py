...
def apply_dense(layer, x):
    if x.dtype.is_complex:
        assert layer.activation is None or layer.activation.__name__ == "linear"
        return Lambda(_complex)(
            [layer(Lambda(tf.math.real)(x)), layer(Lambda(tf.math.imag)(x))]
        )
    else:
        return layer(x)
...
def ncars_inception_graph(
    ...
    features = temporal_convolver.convolve(
        features, filters=filters, temporal_kernel_size=kt0 * 2, activation=activation
    )
    features = layers.BatchNormalization()(features)
    features = dropout(features, dropout_rate)
    ...
    final_logits = Lambda(lambda args: tf.gather(*args), name="final")(
...
def simple_multi_graph(
    ...
    in_stream = comp.SpatialStream(
        grid, times, coords, min_mean_size=5000 if static_sizes else None
    )
    # in_stream = comp.SpatialStream(grid, times, coords, min_mean_size=None)
...
def inception128_multi_graph(
    ...
    filters *= 2
    temporal_convolver = comp.temporal_convolver(
        global_stream, global_stream, decay_time * 4
    )
    features = temporal_convolver.convolve(
        features, filters=filters, temporal_kernel_size=kt0 * 2, activation="relu"
    )
...
def inception_pooling(
    ...
    def do_in_place(in_stream: comp.SpatialStream, features, filters):
        ...
        features = features + branched
        pooled.append(
            in_stream.pool_features(
                t_end, features, pooled_units, pooled_kt, activation=activation
            )
        )
        return features
