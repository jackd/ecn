...
def tuned_simple1d_half_graph(features, labels, weights=None,
...
@gin.configurable(module='ecn.tune')
def tuned_sgd(hp):
    return tf.keras.optimizers.SGD(learning_rate=hp.Float('learning_rate',
                                                          1e-4,
                                                          1.,
                                                          sampling='log'),
...
@gin.configurable(module='ecn.tune')
def tune_multi_graph(build_fn: Callable,
                     ...
                     tuner_fn=Hyperband,
                     optimizer: Optional[
                         Union[tf.keras.optimizers.Optimizer, Callable]] = None,
                     **pipeline_kwargs):
