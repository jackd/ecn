...
def pad_ragged(rt, padding, values=0):
    ...
    padding_tensor = [[0, 0]] * (len(rt.flat_values.shape))
    padding_tensor[0][1] = padding
    padding_tensor = tf.convert_to_tensor(padding_tensor)
    flat_values = tf.pad(rt.flat_values, padding_tensor, constant_values=values)
...
class Grid:
    ...
    def ravel_indices(self, indices: IntTensor, axis=-1):
        ...
        with mg.pre_cache_context():
            return grid_ops.ravel_multi_index(indices, self._shape, axis=axis)
...
class SpatialStream(Stream):
    ...
    def voxelize(
        ...
        with mg.post_batch_context():
            ...
            time = self.frame_indices(
                t_start, t_end, num_frames, dtype=batch_index.dtype
            )
...
class Convolver(Generic[S0, S1]):
    ...
    def gather(self, indices: IntTensor) -> "Convolver":
        ...
        with mg.pre_cache_context():
            ...
            values, splits = ragged_ops.gather_rows(values, self._splits, indices)
    ...
    def _batch_multi_partition(self):
        ...
        components = []
        with mg.pre_cache_context():
            rowids = tf.ragged.row_splits_to_segment_ids(
                self._splits, out_type=self.dtype
            )
            partitions = maybe_cast(self._partitions, tf.int32)

            ijs = tf.dynamic_partition(
                tf_stack(
                    (rowids, self._indices), axis=-1, name="multi_partition_stack"
                ),
                partitions,
                num_partitions,
            )
        ...
        with mg.post_batch_context():
            ...
            for ragged_indices in all_ragged_indices:
                ...
                b = tf.repeat(b, ragged_indices.row_lengths(), axis=0)
            ...
            return tuple(
                tf.SparseTensor(ij, dt, dense_shape) for ij, dt in zip(ijs, dts)
            )
        ##################################
        ########## IMPL 1 END ############
        ##################################
    ...
    def model_dts(self):
        if self._model_dts is None:
            self._model_dts = tuple(mg.model_input(dt) for dt in self.batched_dts)
