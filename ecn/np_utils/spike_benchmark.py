...
kwargs = dict(decay_time=10000, threshold=2, reset_potential=-2.)

manager = BenchmarkManager()

manager.benchmark(times=times.numpy(), name='np',
                  **kwargs)(np_spike.global_spike_threshold)
with tf.device('/cpu:0'):
    # manager.benchmark(times=times, name='tf',
    #                   **kwargs)(tf_spike.global_spike_threshold_tf)
    manager.benchmark(times=times, name='np_wrapped',
                      **kwargs)(tf_spike.global_spike_threshold)
